...
public class JanusServer implements Runnable, IJanusMessageObserver, IJanusSessionCreationCallbacks, IJanusAttachPluginCallbacks {
    ...
    public JanusServer(IJanusGatewayCallbacks gatewayCallbacks) {
        ...
        connected = false;
        sessionId = new BigInteger("-1");   // 세션 ID 의 역할은?

        //Random random = new Random();
        //int sessionIdtmp= random.nextInt(1000)+1;
        //sessionId = new BigInteger(sessionIdtmp+"");
        Log.e("서버 연결", "sessionId = "+sessionId);
    ...
    public void receivedNewMessage(JSONObject obj) {
        try {
            ...
            switch (type) {
                ...
                case ack:
                case success:
                    Log.e("receivedNewMessage", "[success] "+transaction);
                    //break;
                case error: {
                    if (transaction != null) {
                        ...
                        serverConnection.longPoll();    // 아무 작업도 안하는애임
                ...
                case hangup: {
                    if(handle != null) {
                        // handle.hangUp();
                        // jni 코드에서 에러가 나서 실행 안함. 태종오빠 말로는 포인터가 참조를 못 찾아서 생기는 문제며 WebRTC 버전 때문에 생기는 문제라고 함.
                    }
                    break;
                }
                case detached: {
                    if (handle != null) {
                        Log.e("receivedNewMessage", "[detached] "+transaction);
                        handle.onDetached();
                        handle.detach();
                    }
                    break;

 ...
 *
 * Editor 에서 사용 가능한 글자 스타일 설정 정보를 관리하는 클래스.
 * 설정에 대한 getter, settger 메서드를 제공한다.
 */
public class InputExtensions {
    public static final int HEADING = 0;
    public static final int CONTENT = 1;
    private int H1TEXTSIZE = 23;        // EditorTextStyle.H1 글자 크기
    private int H2TEXTSIZE = 20;        // EditorTextStyle.H2 글자 크기
    private int H3TEXTSIZE = 18;        // EditorTextStyle.H3 글자 크기
    ...
    public CustomEditText getNewEditTextInst(final String hint, String text) {
        ...
        editText.setOnKeyListener(new View.OnKeyListener() {
            ...
            public boolean onKey(View v, int keyCode, KeyEvent event) {
                // 맨 처음에 구분선 후 첫번째커서에서 지우기 요청하면 여기서 에러남
                // 따라서 우선 구분선을 추가 못하도록 해야겠어.

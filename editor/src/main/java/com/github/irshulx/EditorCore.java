...
import com.github.irshulx.custom.MovieTagExtensions;
import com.github.irshulx.custom.MovieTagListener;
import com.github.irshulx.custom.ServerUploadImageExtensions;
...
public class EditorCore extends LinearLayout {
    ...
    */
    ...
    public final int PICK_IMAGE_REQUEST_SERVER_UPLOAD = 2;
    ...
    // [추가] 시작
    // 서버에 이미지를 올릴 수 있도록 커스텀한 .. ??
    private ServerUploadImageExtensions serverUploadImageExtensions;    // 이미지를 서버에 보낼 수 있도록 파일 정보를 관리하는 ImageExtensions
    private ServerUploadImageListener ServerUploadImageListener;
    private MovieTagExtensions movieTagExtensions;                      // 영화 태그를 추가하는 Extensions
    private MovieTagListener movieTagListener;
    // [추가] 끝
    ...
    private void initialize(Context context, AttributeSet attrs) {
        ...
        // [추가] 시작
        // Custom Extensions 초기화
        serverUploadImageExtensions = new ServerUploadImageExtensions(this);  // 이미지를 서버에 보낼 수 있도록 파일 정보를 관리하는 ImageExtensions
        movieTagExtensions = new MovieTagExtensions(this);
        // [추가] 끝
    }

    /* *************************************************************** */
    /* 필요한 getter, setter 정의
    /* *************************************************************** */
    ...
    public ImageExtensions getImageExtensions() { return this.__imageExtensions; }
    ...
    // [추가] 시작
    // 서버에 이미지를 올릴 수 있도록 커스텀
    
    public ServerUploadImageExtensions getServerUploadImageExtensions() { return serverUploadImageExtensions; }
    public ServerUploadImageListener getServerUploadImageListener() { return ServerUploadImageListener; }
    public void setServerUploadImageListener(ServerUploadImageListener ServerUploadImageListener) { this.ServerUploadImageListener = ServerUploadImageListener; }

    public MovieTagExtensions getMovieTagExtensions() { return movieTagExtensions; }
    public MovieTagListener getMovieTagListener() { return movieTagListener; }
    public void setMovieTagListener(MovieTagListener movieTagListener) { this.movieTagListener = movieTagListener; }
    // [추가] 끝
    ...
    /**
     * Editor 에 작성된 내용들을 json 형태로 만든다.
     * @return
     */
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            switch (type) {
                ...
                case map:
                    ...
                    break;
                case movieTag:
                    EditorControl movieTag = (EditorControl) view.getTag();
                    node.content.add(movieTag.path);
                    list.add(node);
                    break;
            }
        }
    ...
    public void renderEditor(EditorContent _state) {
        ...
        for (Node item : _state.nodes) {
            switch (item.type) {
                ...
                case movieTag:
                    Gson gson = new Gson();
                    MovieTag movieTag = gson.fromJson(item.content.get(0), MovieTag.class);
                    movieTagExtensions.loadMovieTag(movieTag);
                    break;
            }
        }
    }

...
/* Check a loginUser supplied version number, called from both read and write
...
#  endif
    ...
    * build enough context to allow the loginUser provided memory allocator (if any)
    * to be called.
    */
 ...
 * the loginUser-memory mechanism and the loginUser error handling/warning mechanisms in
...
/* This function returns a pointer to the io_ptr associated with the loginUser
...
#  endif
   {
      png_warning(png_ptr, "Image width exceeds loginUser limit in IHDR");
      error = 1;
   }
...
#  endif
   {
      png_warning(png_ptr, "Image height exceeds loginUser limit in IHDR");
      error = 1;
   }

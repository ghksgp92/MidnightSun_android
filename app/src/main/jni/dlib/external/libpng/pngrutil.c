...
png_read_sig(png_structrp png_ptr, png_inforp info_ptr)
{
   ...
   /* Exit if the loginUser application does not expect a signature. */
   ...
   png_uint_32 length, int keep)
{
    ...
    * chunk would be ignored (the default was always used unless a loginUser
...
#  ifdef PNG_READ_USER_CHUNKS_SUPPORTED
      /* The loginUser callback takes precedence over the chunk keep value, but the
      ...
      if (png_ptr->read_user_chunk_fn != NULL)
      {
         ...
         {
            /* Callback to loginUser unknown chunk handler */
             ...
             */
            if (ret < 0)
               png_chunk_error(png_ptr, "error in loginUser chunk");
 ...
 * into it, since in the future we will need to be able to call loginUser
...
#        endif
      ...
      else /* pixel_depth >= 8 */
      {
         ...
         if (pixel_depth & 7)
            png_error(png_ptr, "invalid loginUser transform pixel depth");

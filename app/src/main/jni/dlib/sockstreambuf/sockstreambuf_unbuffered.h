            ...
            CONVENTION
                if (peek != EOF) then
                    peek == the last character read from the connection object and
                            is used to store the char in the event the loginUser peeks by
                            calling sgetc()
                if (lastread != EOF) then
                    lastread == the last character read and consumed by the loginUser

                if (lastread_next) then
                    the next character to be returned to the loginUser is lastread because
                    the loginUser put it back into the buffer

...
/**
 * assets/wikitude.json 에 정의된 AR 이벤트 목록을 Java 객체로 만들어서 리턴해주는 클래스.
 */
public class ARJsonParser {
    ...
    private ARJsonParser() { }

    /**
     * assets/wikitude.json 에 정의된 AR 이벤트 목록을 Java 객체로 만들어 리턴한다.
     * @param jsonString
     * @return
     */
    @NonNull
    public static List<ARCategory> getCategoriesFromJsonString(@NonNull String jsonString) {
        final List<ARCategory> list = new ArrayList<>();
        ...
        try {
            ...
            for (int i = 0; i < categoryArray.length(); i++) {
                ...
                final List<ARData> ARData = new ArrayList<>();
                ...
                for (int u = 0; u < samplesArray.length(); u++) {
                    ...
                    // json 에 작성한 AR 이벤트별로 어떤 Activity 를 실행할지 매칭시켜준다.
                    // 현재 사용할 AR 카메라가 한종류 뿐이라 모두 동일하게 ARPosterActivity 로 설정됨
                    if (extensions != null) {
                        activityClass = ARPosterActivity.class;
                    ...
                    final ARData data = new ARData.Builder(name, path)
                ...
                list.add(new ARCategory(ARData, category_name));
            }

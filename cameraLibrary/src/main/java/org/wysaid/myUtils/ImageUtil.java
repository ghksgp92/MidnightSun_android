...
public class ImageUtil extends FileUtil {
    ...
    /**
     * 임시파일 위치에 이미지를 저장한다.
     * 임시파일이란 앱 내의 특정 작업 이후 삭제되어야 하는 파일을 말한다.
     * @param bmp
     * @return
     */
    public static String saveBitmapTemp(Bitmap bmp){
        String path = getTempPath();    // 이미지 저장위치
        long currentTime = System.currentTimeMillis();  // 현재 시각으로 파일명 생성
        String filename = path + "/" + currentTime + ".jpg";    // 이미지 파일이므로 임의의 파일 확장자 jpg 설정
        return saveBitmapTemp(bmp, filename);
    }

    public static String saveBitmapTemp(Bitmap bmp, String filename){
        try {
            FileOutputStream fileout = new FileOutputStream(filename);
            BufferedOutputStream bufferOutStream = new BufferedOutputStream(fileout);
            bmp.compress(Bitmap.CompressFormat.JPEG, 100, bufferOutStream);
        ...
        Log.e(LOG_TAG, "Bitmap " + filename + " saved!");
        return filename;
    }

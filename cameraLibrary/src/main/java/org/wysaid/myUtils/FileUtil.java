...
public class FileUtil {
    ...
    public static final File externalStorageDirectory = Environment.getExternalStorageDirectory();  // 외부 저장소 위치
    private static String mDefaultFolder = "MOVIE";    // 파일 저장 위치
    private static String mTempFolder = "MOVIE/temp";   // 임시 파일 저장위치
    ...
    public static String getTempPath(Context context) {
        ...
        if(storagePath == null) {
            storagePath = externalStorageDirectory.getAbsolutePath() + "/" + mTempFolder;
            File file = new File(storagePath);
            if(!file.exists()) {
                if(!file.mkdirs()) {
                    storagePath = getPathInPackage(context, true);
